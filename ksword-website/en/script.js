(() => {
  ...
  function initGitTimeline() {
    ...
    if (title) title.textContent = 'Latest Updates';
    if (status) status.textContent = 'Loading latest updates…';
    ...
    function createCommitItem(commit) {
      ...
      text.textContent = /[\u3400-\u9fff]/.test(commit.message) ? 'Repository update' : commit.message;
    ...
    async function loadCommitsOnce() {
      ...
      try {
        ...
        if (parsed.length === 0) {
          ...
          throw new Error('No commits could be parsed from the GitHub page');
        }

        commits = parsed;
        renderCommits();
        if (status) {
          status.textContent = `Updated · ${formatDate(new Date().toISOString())}`;
        }
      } catch (error) {
        if (status) status.textContent = 'Unable to load updates · Showing cached entries';
